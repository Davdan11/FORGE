...
export default function PlanPage() {
  ...
  return (
    <Page>
      <Screen>
        <Hero image={sessionImage("full")} height="h-[300px]" back="/today" eyebrow={`${goalLabel(plan.goal)} · ${profile.daysPerWeek} days · ${profile.sessionMinutes} min · since ${plan.startDate}`} title={<>Your training.<br /><em>{currentBlock?.name ?? "Block"} block.</em></>}
              ...
              <Section title="The block">
                <div className="card p-4 grid gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="grid gap-0.5 text-sm"><span className="font-medium">Week {currentWeek} · block {blockOfWeek(currentWeek)}</span><span className="text-xs text-smoke">{doneCount} of {sessions.length} sessions · deload every 4th week</span></div>
                  </div>
                  {currentBlock?.review && (
                    <div className="grid gap-1.5 border-t border-line pt-3">
                      <span className="meta">What changed this block</span>
            ...
            <Item>
              <Section title="Why it is built this way">
                <div className="card p-4 grid gap-2 text-sm text-smoke">
                  <p>Four-week blocks, three per cycle, each with its own job. Intensity climbs inside each block; the fourth week backs off so the next one can start higher.</p>
                  <p>At the start of every block, the one before it is reviewed: how many sessions happened and how hard they felt. The loads, the volume and the exercises are rewritten from that. The next block is always ready before you need it.</p>
