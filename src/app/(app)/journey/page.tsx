...
export default function JourneyPage() {
  ...
  const since = new Date(profile.createdAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" });
  const series = [...(weights ?? [])].sort((a, b) => a.date.localeCompare(b.date));

  return (
    <Page>
      <Screen>
        <Hero image={ART.profile} color height="h-[300px]" back="/progress" eyebrow={`Since ${since}`}
          title={<>Your <em>journey.</em></>}>
          <p className="text-sm text-bone/80 max-w-[44ch]">Every week since day one: what you weighed, what you did, what you broke.</p>
        </Hero>

        <Stagger className="grid gap-6">
          <Item>
            <StatRow items={[
              t.changeKg != null ? { label: t.changeKg <= 0 ? "Down since day one" : "Up since day one", value: String(Math.abs(w(t.changeKg))), unit } : { label: "Weigh-ins", value: "0" },
              { label: "Sessions done", value: String(t.sessionsDone) },
              { label: "Records", value: String(t.records) },
            ]} />
          </Item>
            ...
            <Item>
              <Section title="Weight">
                <div className="card p-4 grid gap-2">
                  <div className="flex justify-between text-xs text-smoke tnum"><span>Start · {w(t.startKg ?? series[0].kg)} {unit}</span><span>Now · {w(t.currentKg ?? series.at(-1)!.kg)} {unit}</span></div>
          ...
          <Item>
            <Section title="Week by week" aside={<span className="text-xs text-smoke">{t.activeWeeks} active {t.activeWeeks === 1 ? "week" : "weeks"}</span>}>
                ...
                {journey.weeks.map((wk) => {
                  ...
                  return (
                    <li key={wk.monday} className={`p-4 grid grid-cols-[72px_minmax(0,1fr)] gap-3 ${empty ? "opacity-55" : ""}`}>
                      <span className="meta pt-0.5">{new Date(wk.monday + "T00:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                      <span className="grid gap-1 text-sm">
                        {empty ? <span className="text-smoke">A week off.</span> : (
                          <>
                            <span className="flex flex-wrap gap-x-4 gap-y-1 tnum">
                              {wk.sessionsPlanned > 0 && <span><strong>{wk.sessionsDone}</strong>/{wk.sessionsPlanned} sessions</span>}
                              {wk.activities > 0 && <span><strong>{wk.activities}</strong> {wk.activities === 1 ? "activity" : "activities"} · {fmtDist(wk.distanceM, profile.units)}</span>}
