...
export function autoRegulate(profile: Profile, planned: Session, r: Readiness): Adjustment {
  ...
  if (!constrained && minutes < planned.minutes) {
    ...
    changes.push(`Trimmed to ${minutes} min — one accessory dropped.`);
  }

  /* 3. Readiness scaling of load and volume. */
  let level: Adjustment["level"] = "green";
  let loadMul = 1, volDrop = 0, reason = "Readiness is good. Train as planned; chase the target RPE, not a number.";
  if (r.score < 40) {
    level = "red"; loadMul = 0.8; volDrop = 1;
    reason = `Readiness ${r.score}/100 — sleep ${r.sleepHours}h${r.soreness >= 4 ? ", heavy soreness" : ""}${r.stress >= 4 ? ", high stress" : ""}. Load −20%, one set fewer per exercise. You still show up; the block still moves.`;
  } else if (r.score < 65) {
    level = "amber"; loadMul = 0.92;
    reason = `Readiness ${r.score}/100. Load −8% on the main lifts, same volume. Stop each set one rep earlier than you think.`;
  }
  if (loadMul < 1 || volDrop) {
    ...
    if (session.cardio && level === "red") { session.cardio = { ...session.cardio, zone: 2, minutes: Math.round(session.cardio.minutes * 0.7), structure: undefined }; changes.push("Intervals replaced by easy Zone 2."); }
    changes.push(level === "red" ? "Load −20%, one set fewer." : "Load −8% on loaded work.");
  }
...
export const PAIN_LABEL: Record<PainArea, string> = { knee: "Knee", back: "Lower back", shoulder: "Shoulder", hip: "Hip", wrist: "Wrist", ankle: "Ankle", elbow: "Elbow" };
