...
export function AddToTraining({ ex }: { ex: Pick<Exercise, "slug" | "name"> }) {
  ...
  async function add(id: string, label: string) {
    ...
    setOpen(false);
    setToast(r === "added" ? `Added to ${label}` : r === "already" ? `Already in ${label}` : "That session no longer exists.");
    setTimeout(() => setToast(null), 3000);
  }
  const day = (iso: string) => (iso === today ? "Today" : new Date(iso + "T00:00:00").toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" }));

  return (
    <>
      <Press><button type="button" className="pill pill--volt" onClick={() => setOpen(true)}>+ Add to training</button></Press>
      <AnimatePresence>{open && (
        <motion.div className="fixed inset-0 z-[80] grid items-end lg:place-items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <button type="button" aria-label="Close" className="absolute inset-0 bg-ink/45 backdrop-blur-[2px]" onClick={() => setOpen(false)} />
          <motion.div role="dialog" aria-modal="true" aria-label={`Add ${ex.name} to a session`}
            ...
            <div className="grid gap-1">
              <span className="meta">Add to training</span>
              <p className="display text-2xl leading-none">{ex.name}</p>
              <p className="text-sm text-smoke">Pick a session. Sets, reps and load are set from your current block, and it stays there if the plan is rewritten.</p>
            </div>
            {sessions.length === 0 ? (
              <p className="text-sm text-smoke card p-4">No sessions in the next two weeks. Your plan starts on its first Monday.</p>
            ) : (
                    ...
                    <li key={s.id}>
                      <button type="button" disabled={inIt} onClick={() => add(s.id, `${day(s.date)} · ${s.title}`)}
                        ...
                        <span className="grid min-w-0 flex-1">
                          ...
                          <span className="text-xs text-smoke truncate">{s.title} · {s.minutes} min · {s.exercises.length} movements</span>
                        </span>
                        <span className={`chip shrink-0 ${inIt ? "" : "chip--volt"}`}>{inIt ? "Already in" : "Add"}</span>
                      </button>
                    </li>
                  );
                })}
