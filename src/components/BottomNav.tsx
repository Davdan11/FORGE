        ...
        {ITEMS.map((it) => {
          ...
          return (
            <li key={it.href} className="flex-1 relative">
              {on && <motion.span layoutId="nav-pill" className="absolute inset-1.5 rounded-[20px] bg-[rgba(31,199,111,.1)] border border-[rgba(31,199,111,.22)]" transition={{ type: "spring", stiffness: 420, damping: 34 }} />}
              <Link href={it.href} aria-current={on ? "page" : undefined} onClick={() => navigator.vibrate?.(6)} className={`relative grid justify-items-center gap-1 py-2.5 text-[11px] tracking-[.1em] uppercase transition-colors ${on ? "text-volt" : "text-smoke"}`}>
