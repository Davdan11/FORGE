...
export function SportChallenges({ sport, units }: { sport: ActivityType; units: DistanceUnit }) {
  ...
  const t = useT();
  const lang = useLang();
  // The titles are written in the current language, so a language change rebuilds them.
  const set = useMemo(() => (activities ? challengesFor(sport, today, activities, units) : null), [activities, sport, today, units, lang]); // eslint-disable-line react-hooks/exhaustive-deps
  if (!set) return <div className="skeleton h-40" />;
  const paid = new Set(stats?.challengesDone ?? []);

  return (
    <Section title={`${t("Défis", "Challenges")} · ${sportLabel(sport, lang)}`} aside={<span className="text-xs text-smoke">{t("Remise à zéro chaque lundi", "Weekly resets Monday")}</span>}>
      <div className="grid gap-3">
        <DailyCard c={set.daily} units={units} paid={paid.has(set.daily.id)} t={t} />
        <div className="card divide-y divide-line">
          {set.weekly.map((c) => <WeeklyRow key={c.id} c={c} units={units} paid={paid.has(c.id)} t={t} />)}
        </div>
      </div>
    </Section>
  );
...
function amount(c: Challenge, units: DistanceUnit, tt: T) {
  if (c.unit === "done") return c.done ? tt("Fait", "Done") : tt("Pas encore", "Not yet");
  ...
  const p = unit === "km" ? (c.progress / 1000).toFixed(1) : unit === "mi" ? (c.progress / 1609.344).toFixed(1) : Math.round(c.progress).toLocaleString(locale());
  return `${p} / ${t}`;
}
...
function DailyCard({ c, units, paid, t }: { c: Challenge; units: DistanceUnit; paid: boolean; t: T }) {
  return (
    <div className={`card p-4 grid gap-3 ${c.done ? "!border-volt" : ""}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="grid gap-0.5 min-w-0 flex-1">
          <span className="meta">{t("Défi du jour", "Today’s challenge")}</span>
      ...
      {c.unit !== "done" && (
        <div className="grid gap-1.5">
          ...
          <span className="text-xs text-smoke tnum">{amount(c, units, t)}</span>
        </div>
      )}
    </div>
  );
...
function WeeklyRow({ c, units, paid, t }: { c: Challenge; units: DistanceUnit; paid: boolean; t: T }) {
  return (
